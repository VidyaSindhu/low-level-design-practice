...
public class Application {
  public static void main(String[] args) {
    try {
      ...
      databaseManager.addRow(userTableName, Map.ofEntries(
              Map.entry(nameColumn, "XYZ"),
              Map.entry(ageColumn, 25),
              Map.entry(mobileNumber, "1234567893")
      ));

      databaseManager.addRow(userTableName, Map.ofEntries(
              Map.entry(nameColumn, "ABC"),
              Map.entry(ageColumn, 23),
              Map.entry(mobileNumber, "1234567894")
      ));

      databaseManager.addRow(userTableName, Map.ofEntries(
              Map.entry(nameColumn, "DEF"),
              Map.entry(ageColumn, 25),
              Map.entry(mobileNumber, "1234567895")
      ));
...
//      databaseManager.removeRow(userTableName, 1);
      ...
      databaseManager.filterRows(userTableName, Map.ofEntries(
              Map.entry(ageColumn, new HashSet<>(Arrays.asList(23)))
      ));
